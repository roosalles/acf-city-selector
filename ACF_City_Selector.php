...
if ( ! class_exists( 'ACF_City_Selector' ) ) {
	...
	class ACF_City_Selector {
		...
		public static function acfcs_show_admin_notices() {
			if ( $codes = self::acfcs_errors()->get_error_codes() ) {
				if ( is_wp_error( self::acfcs_errors() ) ) {
					$span_class = false;
					$prefix     = false;
					...
					foreach ( $codes as $code ) {
						$message = self::acfcs_errors()->get_error_message( $code );
						echo '<p class="">';
						echo $message;
						echo '</p>';
					}
		...
		public function acfcs_settings_link( $links ) {
			$settings_link = array( 'settings' => '<a href="options-general.php?page=acfcs-dashboard">' . esc_html__( 'Settings', 'acf-city-selector' ) . '</a>' );
			...
			if ( ! class_exists( 'acf' ) ) {
				add_action(
					...
					function () {
						...
						echo sprintf(
							__( '"%1$s" is not activated. This plugin <strong>must</strong> be activated, because without it "%2$s" won\'t work. Activate it <a href="%3$s">here</a>.', 'acf-city-selector' ),
			...
			if ( isset( $plugins['advanced-custom-fields-pro/acf.php'] ) ) {
				if ( $plugins['advanced-custom-fields-pro/acf.php']['Version'] < 5 && is_plugin_active( 'advanced-custom-fields-pro/acf.php' ) ) {
					add_action(
						...
						function () {
							echo '<div class="notice notice-error"><p>';
							echo sprintf( __( '<b>Warning</b>: The "%1$s" plugin will probably not work properly (anymore) with %2$s v4.x. Please upgrade to PRO.', 'acf-city-selector' ), 'City Selector', 'Advanced Custom Fields' );
							echo '</p></div>';
						}
					);
				}
		...
		public function acfcs_add_scripts() {
			wp_enqueue_style( 'acfcs-general', plugins_url( 'assets/css/general.css', __FILE__ ), array(), $this->settings['version'] );
			if ( is_admin() ) {
				wp_enqueue_style( 'acfcs-admin', plugins_url( 'assets/css/admin.css', __FILE__ ), array(), $this->settings['version'] );
				wp_register_script( 'acfcs-admin', plugins_url( 'assets/js/upload-csv.js', __FILE__ ), array( 'jquery' ), $this->settings['version'] );
