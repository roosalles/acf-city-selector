<?php
	...
	require 'acfcs-field-settings.php';
		...
		while ( ( $csv_line = fgetcsv( $handle, apply_filters( 'acfcs_line_length', 1000 ), "{$delimiter}" ) ) !== false ) {
			...
			if ( count( $csv_line ) != $column_benchmark ) {
				...
				if ( count( $csv_line ) < $column_benchmark ) {
					...
					ACF_City_Selector::acfcs_errors()->add( 'error_no_correct_columns_' . $line_number, sprintf( esc_html__( 'There are too few columns on line %1$d. %2$s', 'acf-city-selector' ), $line_number, $error_message ) );
					...
					ACF_City_Selector::acfcs_errors()->add( 'error_no_correct_columns_' . $line_number, sprintf( esc_html__( 'There are too many columns on line %1$d. %2$s', 'acf-city-selector' ), $line_number, $error_message ) );
				}
			}
		...
		if ( strpos( $file_name, '.csv', -4 ) !== false ) {
			...
			if ( ! is_wp_error( $csv_array ) ) {
				if ( isset( $csv_array['data'] ) && ! empty( $csv_array['data'] ) ) {
					...
					if ( in_array( $file_name, array( 'be.csv', 'nl.csv' ) ) ) {
